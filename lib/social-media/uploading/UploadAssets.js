...
const UploadAsset = ({ assets, resize = true, onChange = () => null }) => {
  const { endpoint, signUrlsEndpoint, directory, axiosinstance } =
    React.useContext(UploadingContext);
  const [uploadingAsset, setUploadingAsset] = React.useState({});
  const [progress, setProgress] = React.useState(0);
  const [error, setError] = React.useState('');

  const strs = assets.map((a) => a.signedUrl || a.Location).join(', ');

  const getUploadUrls = React.useMemo(
    () => async () => {
      try {
        const { data } = await (axiosinstance || axInstance)().post(
          signUrlsEndpoint,
          {
            files: assets,
            ACL: 'public-read',
  ...
  const uploadAsset = React.useMemo(
    () => async (uploadingAsset) => {
      try {
        setError('');
        const uploadedassets = await uploadAssetAsync({
        ...
        if (
          ...
          uploadedassets[0] &&
          uploadedassets[0].Location
        ) {
          onChange(
            assets.map((a) =>
              a.id === uploadingAsset.id ? uploadedassets[0] : a
            )
          );
        }
      } catch (error) {
  ...
  React.useEffect(() => {
    ...
    if (selected) {
      const ind = assets.findIndex((a) => a.id === selected.id);
      setUploadingAsset({
        ...selected,
        index: ind + 1,
      });
  ...
  React.useEffect(() => {
    if (uploadingAsset.id && !uploadingAsset.Location) {
      if (signUrlsEndpoint) {
        if (uploadingAsset.signedUrl) {
  ...
  }, [uploadingAsset.id, uploadingAsset.signedUrl, signUrlsEndpoint]);
  ...
  return (
    <Block>
      {(uploadingAsset.mediaType === 'photo' || uploadingAsset.Location) && (
        <Image
          source={{
            uri:
              uploadingAsset.uri ||
      ...
      {uploadingAsset.mediaType === 'video' && (
        <Video
          source={{
            uri: uploadingAsset.localUri || uploadingAsset.uri,
          }}
          rate={1.0}
          volume={1.0}
          isMuted
      ...
      <Block
        ...
        space="between"
      >
        {Boolean(uploadingAsset.id || uploadingAsset.Location) && !error && (
          <Block
            style={{
          ...
          >
            {uploadCompleted ? (
              ...
              <Block flex={false}>
                <Text white>
                  {`Uploading ${uploadingAsset.index} of ${assets.length} ${progress}%`}
                </Text>
              </Block>
            )}
          </Block>
