...
class ContainedButton extends Component {
  ...
  getBackgroundColor() {
    ...
    let backgroundColor = userColor || theme.primary.main;

    backgroundColor = disabled ? 'rgba(0, 0, 0, 0.12)' : backgroundColor;

    return stateBackgroundColor || backgroundColor;
  }

  getRippleColor() {
    const { rippleColor } = this.props;

    const implementedRippleColor = 'rgba(255, 255,255, 0.56)';
  ...
  handleHover(toggle) {
    ...
    if (color(bgColor).isDark()) {
      ...
      implementedColor = toggle ? color().darken(0.15).rgb().string() : null;
    }
  ...
  render() {
    const { disabled, radius, theme, containerStyle, fullWidth, ...props } =
      this.props;
