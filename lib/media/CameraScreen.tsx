...
export default class CameraScreen extends React.Component {
  ...
  onPictureSaved = async (photo) => {
    ...
    // });
    // this.saveToGallery(photo);
    this.setState({ newPhotos: true, capturedPhoto: photo });
  };
  ...
  saveToGallery = async (photo) => {
    if (photo) {
      ...
      if (status !== 'granted') {
        Alert.alert('Denied CAMERA permissions!');
      }
      if (cameraroll.status !== 'granted') {
        Alert.alert('Denied media library permissions!');
      }

      const savedphoto = await MediaLibrary.createAssetAsync(photo.uri);
      this.props.onSelectOne(savedphoto);
      this.props.onCancel();
    } else {
  ...
  onDone = () => {
    const { capturedPhoto } = this.state;
    if (capturedPhoto) {
      this.saveToGallery(capturedPhoto);
    }
  };
  ...
  changePictureSize = (direction) => {
    ...
    const { length } = this.state.pictureSizes;
    if (newId >= length) {
      newId = 0;
    } else if (newId < 0) {
      newId = length - 1;
  ...
  renderNoPermissions = () => (
    <View style={styles.noPermissions}>
      ...
      >
        <Text color={colors.milkyWhite}>Grant access to camera</Text>
      </Button>
    </View>
  );
      ...
      <View style={styles.pictureSizeContainer}>
        ...
        <View style={styles.pictureSizeChooser}>
          <TouchableOpacity
            ...
            style={{ padding: 6 }}
          >
            <Ionicons name="md-arrow-back" size={14} color="white" />
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            style={{ padding: 6 }}
          >
            <Ionicons name="md-arrow-forward" size={14} color="white" />
          </TouchableOpacity>
        </View>
      </View>
    </View>
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <Image
          source={{
            uri: capturedPhoto.uri || './assets/no_available_image.png',
          }}
        ...
        <View style={styles.headerBar}>
          ...
          <Button
            ...
            rounded
            onPress={this.onDone}
            color={`rgba(${hexToRgb(Colors.green30)},0.8)`}
          >
            <Text style={{ ...styles.textStyle, color: '#FFFCFF' }}>Done</Text>
          </Button>
        </View>
        ...
        >
          <Button
            small
            rounded
            color={`rgba(${hexToRgb(Colors.red30)},0.4)`}
            onPress={this.onDelete}
          >
            <Text style={{ color: '#FFF6FF' }}>delete</Text>
          </Button>
        </View>
      </View>
    );
