...
import Modal from '../modal/BlockModal';
...
const RemoteSelector = ({
  ...
  const [options, setOptions] = React.useState(selectedValue || []);
  ...
  const logChange = (val) => {
    if (isMulti) {
      if (!val) {
        ...
        const ind = selectedValue.findIndex((item) => item.key === val.key);

        let values = [];
        if (ind === -1) {
          values = [...selectedValue, val];
        } else {
          values = [...selectedValue].filter((item) => item.key !== val.key);
        }
