...
export default class Dropdown extends PureComponent {
  ...
  onSelect(item, index) {
    const { valueExtractor, onChange, onSelectChange, animationDuration } =
      this.props;
  ...
  renderBase(props) {
    ...
    return (
      ...
      >
        ...
        {title && <Text style={{ flex: 1, color: '#333' }}>{`${title} `}</Text>}
  ...
  renderItem({ item, index }) {
    ...
    return (
      <Block ripple onPress={() => this.onSelect(item, index)}>
        <Button
